        ...
        for step in range(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
            ...
            if step % 1000 == 0:
                valid_feed_dict = fill_feed_dict(valid_data, valid_labels,
                                                 images_placeholder, labels_placeholder, step)
                valid_summary, _, l, lr, valid_acc = sess.run([merged, optimizer, loss, learning_rate, accuracy],
                                                              feed_dict=valid_feed_dict, options=run_options,
...
def main():
    prepare_log_dir()
    train_data, train_labels = load_svhn_data("train")
    valid_data, valid_labels = load_svhn_data("valid")
    test_data, test_labels = load_svhn_data("test")
